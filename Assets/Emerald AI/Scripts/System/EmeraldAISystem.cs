...
namespace EmeraldAI
{
    ...
    public class EmeraldAISystem : MonoBehaviour
    {
        ...
        // Define a delegate for methods that will handle the death event
        ...
        {
            CombatStateRef = EmeraldAISystem.CombatState.NotActive;
        }
        public void ActiveCombatState()
        {
            CombatStateRef = EmeraldAISystem.CombatState.Active;
        }

        public void EnableCocktale()
        {
            
            EmeraldEventsManagerComponent.FrostyCocktail();
        }
        public void BackToCombat()
        {
            EmeraldBehaviorsComponent.ActivateCombatState();
            EmeraldEventsManagerComponent.StopFrostyCocktail();
        }

        public void EnableSmartPhone()
        {
            EmeraldEventsManagerComponent.LoopEmoteAnimation(0);
        }
        public void DisableSmartPhone()
        {
            EmeraldBehaviorsComponent.ActivateCombatState();
            EmeraldEventsManagerComponent.StopLoopEmoteAnimation(0);
        }
        ...
        void Update()
        {
            ...
            if (Input.GetKeyDown(KeyCode.D) || PlayerPrefs.GetInt("DaneceAbility" + transform.gameObject.name + transform.gameObject.GetInstanceID()) == 0 && PlayerPrefs.GetInt("DaneceAbilityActive") == 1)
            {
                PlayerPrefs.SetInt("DaneceAbility" + transform.gameObject.name + transform.gameObject.GetInstanceID(), 1);
                PlayerPrefs.SetInt("Danece", 1);
                EmeraldEventsManagerComponent.LoopEmoteAnimation(0);

                // EmeraldEventsManagerComponent.PlayEmoteAnimation(0);
            }
            if (Input.GetKeyDown(KeyCode.F) || PlayerPrefs.GetInt("DaneceAbility" + transform.gameObject.name + transform.gameObject.GetInstanceID()) == 1 && PlayerPrefs.GetInt("DaneceAbilityActive") == 0)
            {
                PlayerPrefs.SetInt("DaneceAbility" + transform.gameObject.name + transform.gameObject.GetInstanceID(), 0);

                PlayerPrefs.SetInt("Danece", 0);
                EmeraldBehaviorsComponent.ActivateCombatState();
                EmeraldEventsManagerComponent.StopLoopEmoteAnimation(0);
            }

            //if (Input.GetKeyDown(KeyCode.D) || PlayerPrefs.GetInt("FreezAbility" + transform.gameObject.name + transform.gameObject.GetInstanceID()) == 0 && PlayerPrefs.GetInt("FreezAbilityActive") == 1)
            //{
            //    PlayerPrefs.SetInt("FreezAbility" + transform.gameObject.name + transform.gameObject.GetInstanceID(), 1);
            //    PlayerPrefs.SetInt("Freez", 1);
            //    EmeraldEventsManagerComponent.LoopEmoteAnimation(1);

            //    // EmeraldEventsManagerComponent.PlayEmoteAnimation(0);
            //}
            //if (Input.GetKeyDown(KeyCode.F) || PlayerPrefs.GetInt("FreezAbility" + transform.gameObject.name + transform.gameObject.GetInstanceID()) == 1 && PlayerPrefs.GetInt("FreezAbilityActive") == 0)
            //{
            //    PlayerPrefs.SetInt("FreezAbility" + transform.gameObject.name + transform.gameObject.GetInstanceID(), 0);

            //    PlayerPrefs.SetInt("Freez", 0);
            //    EmeraldBehaviorsComponent.ActivateCombatState();
            //    EmeraldEventsManagerComponent.StopLoopEmoteAnimation(0);
            //}
            //Debug.Log("DistanceFromTarget" + DistanceFromTarget + transform.name);
            ...
            if (CurrentTarget != null && m_EmeraldAIAbility != null)
            {
                ...
                if (AdjustedAngle <= 180 && !EmeraldLookAtComponent.LookAtInProgress)
                {
                    ...
                    else if (m_EmeraldAIAbility.AbilityType == EmeraldAIAbility.AbilityTypeEnum.Support && SupportAbilities.Count > 0 && !HealingCooldownActive) //Support
                    {
                        ...
                        if(AbilityEffect.transform.GetComponentInChildren<ProjectileNew>())
                        AbilityEffect.transform.GetComponentInChildren<ProjectileNew>().shootByEnemy = true;
        ...
        public void Damage (int DamageAmount, TargetType? TypeOfTarget = null, Transform AttackerTransform = null, int RagdollForce = 100, bool CriticalHit = false)
        {
            ...
            if (CurrentHealth <= 0 && !IsDead)
            {
                ...
                OnDeath();
                GameEventsHandler.Instance.onKillEnemy?.Invoke(GetComponent<EnemyAIDeath>().enemyType);
            }
        }
        ...
        public void RemoveEnemy(GameObject EnemyT)
        {
            WaveManager.WMmanager.RemoveEnemy(EnemyT);
        }

        public void MakeEnemyFriendly()
        {
            CurrentTarget = null;

            EmeraldEventsManagerComponent.SetFactionLevel("Creature", "Enemy");
            EmeraldEventsManagerComponent.SetFactionLevel("NPC", "Enemy");

            EmeraldEventsManagerComponent.SetPlayerRelation(PlayerFactionClass.RelationType.Friendly);
            EmeraldEventsManagerComponent.ChangeFaction("NPC");
        }
        public void MakeEnemyAggressive()
        {
            CurrentTarget = null;

            EmeraldEventsManagerComponent.SetFactionLevel("Creature", "Friendly");
            EmeraldEventsManagerComponent.SetFactionLevel("NPC", "Friendly");

            EmeraldEventsManagerComponent.SetPlayerRelation(PlayerFactionClass.RelationType.Enemy);
            EmeraldEventsManagerComponent.ChangeFaction("Creature");
        }
        public void ChangeToOldFaction()
        {
            Debug.Log("Rooling Back  Faction");
           // EmeraldEventsManagerComponent.SetFactionLevel("Undead", "Enemy");
            EmeraldEventsManagerComponent.SetFactionLevel("Creature", "Friendly");
