...
namespace EmeraldAI.Utility
{
    public class EmeraldAIProjectile : MonoBehaviour
    {
        ...
        void Update()
        {
            ...
            if (!AngleTooBig && !TargetLost)
            {
                    ...
                    !TargetInView && !Collided && ProjectileDirection != Vector3.zero)
                {
                    ...
                    Vector3 targetXZPos = new Vector3(ProjectileCurrentTarget.position.x, 0.0f, ProjectileCurrentTarget.position.z);
                    ...
                    float tanAlpha = Mathf.Tan(35f * Mathf.Deg2Rad);
                    //float H = (TargetObjectTF.position.y + GetPlatformOffset()) - transform.position.y;

                    // calculate the local space components of the velocity 
                    // required to land the projectile on the target object 
                    float Vz = Mathf.Sqrt(G * R * R / (2.0f * (0f - R * tanAlpha)));
                    float Vy = tanAlpha * Vz;

                    // create the velocity vector in local space and get it in global space
                    Vector3 localVelocity = new Vector3(0f, Vy, Vz + 5);
                    Vector3 globalVelocity = transform.TransformDirection(localVelocity);

                    //// launch the object by setting its initial velocity and flipping its state
                    // Update position with adjusted trajectory and gravity
                    transform.position += (globalVelocity.normalized * Time.deltaTime * ProjectileData.ProjectileSpeed) - ProjectileGravity;
                    transform.rotation = Quaternion.LookRotation(AdjustTargetPosition);
                }
        ...
        void OnTriggerEnter(Collider C)
        {
            ...
            if (!Collided && EmeraldComponent != null && ProjectileCurrentTarget != null && C.transform.IsChildOf(ProjectileCurrentTarget.transform) && !C.transform.IsChildOf(EmeraldComponent.transform) && C.gameObject.layer != 2 && !C.isTrigger)
            {
                ...
                else if (TargetTypeRef == EmeraldAISystem.TargetType.Player) //Damage the Player with damage based off of currently applied ability
                {
                    if (ProjectileData.AbilityType == EmeraldAIAbility.AbilityTypeEnum.Damage && ProjectileData.DamageType == EmeraldAIAbility.DamageTypeEnum.Instant)
                    {
                        if (!PlayerControllerS.PlayerControllerSInstance.isCrouched)
                        {
                            DamagePlayer(Damage);
                        }
                    }
        ...
        {
            if (StartingTarget.GetComponent<PlayerControllerS>())
            {
                Debug.Log("PlayerControllerS+Found");
                StartingTarget.GetComponent<PlayerControllerS>().DecreaseHealth(SentDamage);
            }
